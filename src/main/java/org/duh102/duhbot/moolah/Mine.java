...
public class Mine {
  ...
  public Pair<Double, Long> genMineRichness() {
    ...
    Double richness = Math.pow(10.0, (currGauss/3.0+2.0))+1; //richness in $ per mining chunk
    ...
    Long time = Math.round((AVG_UPDATE/Math.max(Math.pow(2, ((currGauss+1)/2)), 0.025))/2.0)+1; // in ms
    return new Pair<>(richness, time);
  }
